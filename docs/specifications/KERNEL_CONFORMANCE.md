...
- **Mandatory scenario:** a test whose identifier appears in Sections 12–27 or the adversarial matrix. Every mandatory scenario MUST pass unless its requirement is formally inapplicable to a constitutionally valid implementation; the conformance report MUST justify and independently approve any inapplicability. The suite contains 206 catalog identifiers; a parameterized row may require multiple executions without changing that catalog count.
...
- one complete consequential Decision conforming to `DECISION_RECORD.md`; and
...
Every fixture export MUST include exact specification versions, stable identifiers, entity versions, complete initial authoritative Event streams with stream positions, expected baseline projections, Policy contents and precedence, controlled external reference manifests, integrity identifiers, and classification labels. Fixtures MUST NOT rely on the current wall clock, random identity generation during a test, ambient model state, or undisclosed prior Events.
...
| Expected Events | Event types and all normative fields in exact logical order, including recording Command, causal reference, epistemic status, and stream positions relative to baseline |
...
Event comparison MUST use exact semantic equality for Event type, schema version, organization, initiating Actor, recording Command, correlation, causal reference, Resource references, result, evidence, epistemic status, conditional confidence, payload semantics, and audit-critical references. It MUST use stable ordering equality for organization stream position and all within-Command Event order.
...
Two projections are equivalent only when all normative entities, identifiers, versions, lifecycle states, Work Roots, relationships, Policy versions, authority and delegation state, Approval usage, Resource reservations and consumption, schedules, memory validity and supersession, Incident controls, audit links, and governed external references are semantically equal.

Index layout, cache entries, precomputed views, internal timestamps unrelated to normative Events, and performance metadata are ignored only if they cannot affect a later admission or disclosure. Projection comparison MUST validate the source Event history and last applied stream position; equal-looking projections built from corrupt, missing, reordered, or unknown Events do not pass.
...
The minimum suite contains 206 mandatory scenarios. Every scenario MUST be instantiated using the normative test-case format in Section 4.5.
...
| Bootstrap | `BST-001`–`BST-010` | 10 | Atomic first-Organization establishment and bootstrap authority containment |
| Command admission | `CMD-001`–`CMD-014` | 14 | Validation, attribution, isolation, versions, idempotency, and evaluation time |
| Authority and Policy | `AUT-001`–`AUT-014` | 14 | Explicit authority, delegation, precedence, and human-reserved power |
| Work Root | `WRT-001`–`WRT-008` | 8 | Exclusive Goal-or-duty traceability |
...
| Scheduling and orchestration | `SCH-001`–`SCH-012` | 12 | Attributable materialization and current-state revalidation |
...
| Audit and Decisions | `AUD-001`–`AUD-012` | 12 | Complete consequential trace and collective attribution |
| Replay and recovery | `RPL-001`–`RPL-012` | 12 | Projection equivalence, history integrity, and zero effects |
| Portability and model replacement | `POR-001`–`POR-008` | 8 | Institutional continuity across environments and replaceable models |
| **Total** |  | **206** | All scenarios are independently mandatory |
...
|---|---|---|
| BST-001 | Bootstrap first Organization with verified Human and complete founding set | Atomically accept Organization, Human, owner/governor Role, assignment, founding Decision and duty, Grants, Events, stream positions, and audit links |
| BST-002 | Fail append after preparing only part of bootstrap | No operational Organization or partial authoritative entity exists; no ordinary Command is admitted |
...
| BST-004 | Founding set omits Role, assignment, Decision, Grant, Event, or audit link | Reject atomically; no component becomes active |
| BST-005 | Submit operational Command while bootstrap is incomplete | Reject with bootstrap-incomplete reason; no work, dispatch, or Resource use |
| BST-006 | Redeliver exact bootstrap Command and idempotency key | Return original disposition and identifiers; do not create a second Organization or founding Event set |
| BST-007 | Reuse bootstrap idempotency key with conflicting owner or Organization data | Reject conflict; preserve original bootstrap unchanged |
...
| BST-010 | Attempt to reuse bootstrap authority for an operational Action | Reject under ordinary authority rules; bootstrap power ended at establishment |
...
| AUT-012 | Employee attempts to complete A4 or constitutionally reserved Decision | Reject and route to eligible Human without recording Employee as decider |
...
| ID | Scenario and distinguishing input | Required observable result |
|---|---|---|
| SCH-001 | Approved schedule becomes due | Materialize one attributable Command with scheduler Service, schedule trigger, correlation, and audit references |
...
| SCH-004 | Policy or Authority changed after schedule creation | Evaluate current versions at materialization; reject or constrain under current rules |
...
| SCH-008 | Same schedule instance is delivered twice | Idempotently materialize one Command and no duplicate dispatch |
...
| EVT-010 | Event has missing epistemic status or required confidence | Reject schema/epistemic contract violation |

## 22. Subscription isolation test suite
...
## 23. Memory governance test suite
...
## 24. Suspension, cancellation, timeout, retry, and escalation test suite
...
## 25. Audit and consequential Decision test suite
...
| AUD-007 | Governing Body makes collective Decision | Preserve each Human vote, dissent, consent, or recusal; verify current quorum/Policy; derive outcome |
...
## 26. Replay and recovery test suite
...
## 27. Portability and model-replacement test suite
...
## 28. Required evidence from a conforming test run
...
## 29. Conditions invalidating conformance
