...
The Organization is the tenancy, isolation, governance, and Event-ordering boundary. The kernel MUST NOT require or infer a separate Tenant entity or `tenant_id` for ordinary organizational admission.
...
### Authenticated recording boundary

Ordinary post-genesis admission has a distinct boundary between untrusted input processing and attributable Organization processing. A syntactically valid `organization_id` or `initiating_actor_id` is only a claim. Before any Organization stream read, Organization-scoped idempotency lookup, authoritative identifier allocation, audit creation, or Event append, a trusted capability-neutral `RecordingBoundaryResolver` MUST return `AdmissionEstablished` for the exact immutable `AdmissionClaim`.

`AdmissionClaim` binds the submitted message, Command, claimed Organization, claimed initiating Actor, and invocation-proof reference. `AdmissionEstablished` proves that the exact Organization exists as a completed-genesis authoritative boundary; the exact stable Actor exists and is validly attributable within that Organization; the invocation proof authenticates that Actor for this Command attempt; and the proof is supported by immutable Organization-genesis, Actor-identity, authentication-evidence, and admission-mechanism references and versions. Returned Organization and Actor identifiers MUST equal the submitted stable identifiers. Resolution by alias, display name, email, username, external account identifier, cross-Organization search, or fallback is prohibited at this boundary.

`AdmissionEstablished` proves authentication and attribution only. It grants no Authority, Role, capability, Approval, Decision, or Policy eligibility. Authorization remains in the later Role, Authority, Policy, Decision, Approval, Resource, and lifecycle gates. A domain handler MUST NOT receive the resolver or interpret authentication evidence.

`AdmissionDenied` is a typed, non-authoritative result containing a stable reason code, failed admission gate, bounded safe detail, and optional non-authoritative diagnostic facts. It does not establish that the claimed Organization exists. A pre-boundary rejection has no authoritative disposition identifier, audit record, recorded Event, Organization idempotency record, stream read, append, governance invocation, or domain-handler invocation. Repetition of the same denied claim is another effect-free attempt, not Organization-scoped exact redelivery.

Organization existence as an ordinary recording boundary requires completed constitutional genesis. Capability-specific constitutional eligibility remains governance. Bootstrap uses its reserved pre-Organization constitutional admission contract and MUST NOT enter this ordinary resolver path; ordinary Commands cannot claim bootstrap mode.

Support resolution follows **Model A**: after effect-free decoding validates field structure, the kernel resolves whether the Command schema, operation type, and operation version are supported before invoking the recording-boundary resolver. Unsupported schema, operation, or version returns a typed non-recorded rejection. This prevents hostile or obsolete type claims from probing or mutating a named Organization. Command-specific domain validation remains after authenticated admission and governance; effect-free payload-schema validation necessary to select and safely decode the operation remains pre-boundary.

An injected authoritative clock MAY supply one evaluation time before boundary resolution because reading it is effect-free. Failure to obtain valid time returns a non-recorded rejection. That time becomes authoritative Organization history only if a later attributable disposition is recorded. No authoritative Event, Audit Record, or Organization disposition identifier may be allocated before `AdmissionEstablished`.

1. **Parse without effect.** Bound input size, decode the envelope, validate immutable field structure and payload decodability, and reject malformed input without reading an Organization stream, invoking governance or a handler, allocating authoritative identifiers, or changing governed state.
2. **Validate schema, operation, and version support without effect.** Require a known Command and payload schema, supported operation type and version, required fields, field semantics, stable identifier syntax, and an allowed migration path. Unknown, unsupported, or ambiguous types and versions fail before Organization admission and are not recorded authoritatively.
3. **Resolve organization boundary.** Through the trusted recording-boundary resolver, confirm exact stable Organization identity, completed genesis, and eligibility as an authoritative ordinary recording boundary. A well-formed identifier is not proof, and denial performs no Organization read or mutation.
4. **Authenticate attribution.** Through the same closed admission resolution, resolve exactly one stable initiating Actor, validate its attribution within the exact Organization, and verify invocation authenticity. Initiation does not imply deciding, approving, causal, or operational authority.
...
10. **Validate lifecycle and concurrency.** Confirm each target's current version and state, dependency states, legal transition, suspension and Incident conditions, applicable approval requirements, and expected Organization stream position. For entity-scoped operations, entity state and revision are additional domain preconditions evaluated from the same bound Organization history; they do not replace Organization-stream concurrency. Stale or conflicting state rejects the Command for reevaluation.
...
13. **Atomically append admission outcome.** In one indivisible Organization-stream commit, verify the expected prior Organization stream position, assign new positions, and append immutable acceptance and state-transition Events, Resource reservation Events, Approval-use facts, audit links, and any authorized dispatch intent. All authoritative post-genesis entity Events, including Role Events, use this single monotonically ordered Organization stream. A concurrent conflict returns to reevaluation; no independent entity stream may override it. Either the complete set becomes authoritative or none of it does. Every Event identifies the recording Command; its causal-reference field remains semantically distinct from that Command and may be null only where [`EVENT_MODEL.md`](EVENT_MODEL.md) permits an independently initiated internal Command. A recorded Approval use is monotonic and MUST NOT be decremented or silently restored; uncertainty is reconciled, and any replacement permission requires a new valid Approval or other explicitly permitted disposition.
...
No earlier step may be treated as evidence that a later step passed. Only `AdmissionEstablished` permits Organization-scoped reads, idempotency, concurrency inspection, governance, authoritative audit, or append. A Command rejected at a later gate MUST produce an attributable rejection Event when the established boundary, current expected position, and append availability permit it. A concurrency conflict cannot be recorded into the stale expected append; an append failure cannot claim its own durable rejection; an idempotency conflict preserves the original registration and records no replacement. Invalid existing history fails closed and requires the applicable integrity or reconciliation disposition rather than appending through untrusted state. Input too malformed, hostile, unsupported, unresolved, or unattributable to admit as an AIOS Event MAY produce non-authoritative platform security telemetry under separate protective controls, but it MUST NOT enter an Organization stream, affect AIOS state, or be misattributed to an Organization or Actor.
