...
| Decisions and Approvals | Validate Decision completeness, approval eligibility, mode, scope, usage, and current conditions | Frame and record alternatives, evidence, rationale, risks, costs, benefit, and outcome | Carry only approval and Decision references required by the invocation | May record external authorization, but it is not an AIOS Approval unless admitted |
...
| Scheduling | Materialize approved schedules as attributable Commands when due; enforce timeouts and expiry | Propose timing, dependencies, priority, and changes | Execute only an admitted dispatch | Supply clocks, calendars, or delivery facilities without creating authority |
...
12. scheduling, orchestration, timeout, cancellation, retry eligibility, reconciliation, and escalation state;
...
Employees remain accountable institutional Actors for planning, evidence gathering, proposals, execution choices, and result assessment. Model instances are replaceable computational Resources used by Actors; they are never institutional Actors, Authority Grant holders, approving Humans, or sources of Policy.
...
9. **Validate Decision and Approval.** Require the consequential Decision record when applicable. Validate exact Decision version, approver eligibility, collective dispositions and quorum when applicable, Approval mode, scope, assumptions, effective and expiry conditions, revocation triggers, review status, and separation from Authority.
...
- its schema and epistemic contract are valid;
- its organization, initiator, correlation, causal reference, Resources, evidence, and result fields are explicit;
...
The kernel may coordinate accepted Tasks, dependencies, timeouts, expiry, reviews, retries, subscriptions, adapters, Employees, and workers. Scheduling is governance, not work creation or reasoning.
...
When a trigger becomes due, the kernel or an authorized scheduling Service MUST issue a new recording Command that cites the schedule and timer or deadline as its trigger. The kernel MUST re-evaluate current identity, Role, authority, Policy, Work Root, Approval, Resource, lifecycle, and suspension state. A schedule is not durable authority and MUST NOT bypass changed conditions.
...
The first Organization is established by one constitutional bootstrap transaction initiated by a verified Human exercising the Human-reserved establishment power. The transaction MUST atomically create and link:
...
- the founding Decision with a complete constitutional `duty_reference`;
- initial Authority Grant or Grants limited to the initial lawful governance scope;
- founding Events with one recording Command, initiating Actor, causal references, and organization stream positions; and
- Audit Record references sufficient to reconstruct the transaction.

The kernel MUST validate the complete post-transaction invariant set before making any component active. A failed pre-append bootstrap set is discarded without authoritative state change. Any detected partial authoritative bootstrap is an invariant and integrity failure that quarantines the affected stream pending accountable review; it is never an operational Organization. No operational Command, subscription, Employee, worker, Tool invocation, or Resource consumption is permitted before bootstrap completes.

Bootstrap authority exists only for the Human-reserved act of establishment and initial governance. It MUST NOT be reused as standing operational authority, inferred for another Human or Organization, or used to bypass ordinary Role, Grant, Policy, Approval, Event, lifecycle, and audit rules after completion.
...
| Bootstrap | Partial creation, ordinary Command during bootstrap, reused bootstrap authority, or invalid founding links | No active partial Organization; complete atomically or quarantine; ordinary rules after completion |
